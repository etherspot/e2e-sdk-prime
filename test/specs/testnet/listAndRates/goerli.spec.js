...
describe('The PrimeSDK, when get the NFT List, Token List and Exchange Rates details with goerli network on the TestNet', function () {
  ...
  it('SMOKE: Validate the Token List on the goerli network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        ...
        let tokenListTokens;
        try {
          tokenLists = await goerliDataService.getTokenLists({ chainId: data.goerli_chainid });
          ...
          tokenListTokens = await goerliDataService.getTokenListTokens({ chainId: data.goerli_chainid });
  ...
  it('SMOKE: Validate the Exchange Rates on the goerli network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        ...
        let requestPayload;
        try {
          TOKEN_LIST = [data.tokenAddress_goerliUSDC, data.tokenAddress_goerliUSDT];
  ...
  it('REGRESSION: Validate the Token List with different argument in the on the goerli network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
  ...
  it('REGRESSION: Validate the Exchange Rates with other token address on the goerli network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
  ...
  it('REGRESSION: Validate the Exchange Rates with incorrect token address on the goerli network', async function () {
    ...
    if (runTest) {
      await customRetryAsync(async function () {
        ...
        try {
          TOKEN_LIST = [
            data.incorrectTokenAddress_goerliUSDC,
            data.tokenAddress_goerliUSDT,
          ];
  ...
  it('REGRESSION: Validate the Exchange Rates without token address on the goerli network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
