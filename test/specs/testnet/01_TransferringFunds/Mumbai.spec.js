...
describe('The PrimeSDK, when transfer a token with Mumbai Network on the TestNet', function () {
  ...
  it('REGRESSION: Perform the transfer native token with the incorrect TxHash while getting the transaction hash on the Mumbai Network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
        ...
        try {
          ...
          while (userOpsReceipt == null && Date.now() < timeout) {
            await Helper.wait(500);
            userOpsReceipt = await mumbaiTestNetSdk.getUserOpReceipt(
  ...
  it('REGRESSION: Perform the transfer native token with the past TxHash while getting the transaction hash on the Mumbai Network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
