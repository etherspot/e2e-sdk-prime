...
describe('Perform the transaction with arka and pimlico paymasters on the TestNet (with new wallet)', function () {
        ...
        await customRetryAsync(async function () {
          ...
          if (utils.isAddress(paymasterAddress)) {
            ...
            try {
              ...
              contract = await testnetPrimeSdk.addUserOpsToBatch({
            ...
            try {
              approveOp = await testnetPrimeSdk.estimate();
            ...
            // get the uoHash1
            try {
              uoHash1 = await testnetPrimeSdk.send(approveOp);
            ...
            while (userOpsReceipt1 == null && Date.now() < timeout1) {
              helper.wait(data.mediumTimeout);
              userOpsReceipt1 = await testnetPrimeSdk.getUserOpReceipt(uoHash1);
            }
      ...
      if (runTest) {
        await customRetryAsync(async function () {
          ...
          if (utils.isAddress(paymasterAddress)) {
            ...
            // get the uoHash1
            try {
              await testnetPrimeSdk.send(approveOp);
            } catch (e) {
              console.error(e);
              const eString = e.toString();
              if (eString === 'Error') {
