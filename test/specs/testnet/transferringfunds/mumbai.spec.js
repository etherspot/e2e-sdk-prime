...
describe('The PrimeSDK, when transfer a token with Mumbai Network on the TestNet', function () {
  ...
  it('SMOKE: Perform the transfer native token by passing callGasLimit with valid details on the mumbai network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
        ...
        let op;
        try {
          op = await mumbaiTestNetSdk.estimate(null, null, 40000);
  ...
  it('REGRESSION: Perform the transfer native token by passing callGasLimit with the incorrect To Address while estimate the added transactions to the batch on the mumbai network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
        ...
        // passing callGasLimit as 40000 to manually set it
        try {
          await mumbaiTestNetSdk.estimate(null, null, 40000);
  ...
  it('REGRESSION: Perform the transfer native token by passing callGasLimit with the invalid To Address i.e. missing character while estimate the added transactions to the batch on the mumbai network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
  ...
  it('REGRESSION: Perform the transfer native token by passing callGasLimit without adding transaction to the batch while estimate the added transactions to the batch on the mumbai network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
