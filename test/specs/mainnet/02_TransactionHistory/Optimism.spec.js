...
describe('The PrimeSDK, when get the single transaction and multiple transaction details with optimism network on the MainNet', () => {
  ...
  xit('SMOKE: Validate the transaction history of the native token transaction on the optimism network', async () => {
    if (runTest) {
  ...
  it('REGRESSION: Validate the get transactions history response with invalid chainid in optimism network', async () => {
    if (runTest) {
      try {
  ...
  it('REGRESSION: Validate the get transactions history response with incorrect chainid in optimism network', async () => {
    if (runTest) {
      try {
        await optimismMainNetSdk.getTransactions({
  ...
  it('REGRESSION: Validate the get transactions history response with invalid account in optimism network', async () => {
    if (runTest) {
      try {
        await optimismMainNetSdk.getTransactions({
          chainId: Number(process.env.OPTIMISM_CHAINID),
  ...
  it('REGRESSION: Validate the get transactions history response with incorrect account in optimism network', async () => {
    if (runTest) {
      try {
        await optimismMainNetSdk.getTransactions({
          chainId: Number(process.env.OPTIMISM_CHAINID),
