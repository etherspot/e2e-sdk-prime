...
describe('The PrimeSDK, when get the single transaction and multiple transaction details with optimism network on the MainNet', () => {
  beforeEach(async () => {
    // added timeout
  ...
  it('SMOKE: Validate the transaction history of the native token transaction on the optimism network', async () => {
    if (runTest) {
  ...
  it('SMOKE: Validate the NFT List on the optimism network', async () => {
    if (runTest) {
      let nfts;
      try {
        nfts = await optimismMainNetSdk.getNftList({
          chainId: Number(process.env.OPTIMISM_CHAINID),
          account: data.sender,
        });
      ...
      console.warn(
        'DUE TO INSUFFICIENT WALLET BALANCE, SKIPPING TEST CASE OF THE NFT LIST ON THE optimism NETWORK',
      );
    }
  });

  xit('REGRESSION: Validate the get transactions history response with random hash in optimism network', async () => {
    if (runTest) {
  ...
  it('REGRESSION: Validate the get transaction history response with invalid hash on optimism network', async () => {
    if (runTest) {
      // Fetching a single transaction
      let transaction;
      try {
        transaction = await optimismMainNetSdk.getTransaction({
  ...
  it('REGRESSION: Validate the get transaction history response when hash hex is not with 32 size on optimism network', async () => {
    if (runTest) {
      // Fetching a single transaction
      try {
        try {
  ...
  xit('REGRESSION: Validate the get transactions history response with invalid chainid in optimism network', async () => {
    if (runTest) {
      try {
  ...
  xit('REGRESSION: Validate the get transactions history response with incorrect chainid in optimism network', async () => {
    if (runTest) {
      try {
        await optimismMainNetSdk.getTransactions({
  ...
  xit('REGRESSION: Validate the get transactions history response with invalid account in optimism network', async () => {
    if (runTest) {
      try {
        await optimismMainNetSdk.getTransactions({
          chainId: Number(process.env.OPTIMISM_CHAINID),
  ...
  xit('REGRESSION: Validate the get transactions history response with incorrect account in optimism network', async () => {
    if (runTest) {
      try {
        await optimismMainNetSdk.getTransactions({
          chainId: Number(process.env.OPTIMISM_CHAINID),
  ...
  it('REGRESSION: Validate the NFT List with invalid chainid on the optimism network', async () => {
    if (runTest) {
      try {
        await optimismMainNetSdk.getNftList({
          chainId: process.env.INVALID_OPTIMISM_CHAINID,
          account: data.sender,
        });
      ...
      console.warn(
        'DUE TO INSUFFICIENT WALLET BALANCE, SKIPPING TEST CASE OF THE HISTORY OF THE NFT LIST WITH INVALID CHAINID ON THE optimism NETWORK',
      );
    }
  });

  it('REGRESSION: Validate the NFT List with invalid account address on the optimism network', async () => {
    if (runTest) {
      try {
        await optimismMainNetSdk.getNftList({
          chainId: Number(process.env.OPTIMISM_CHAINID),
      ...
      console.warn(
        'DUE TO INSUFFICIENT WALLET BALANCE, SKIPPING TEST CASE OF THE HISTORY OF THE NFT LIST WITH INVALID ACCOUNT ON THE optimism NETWORK',
      );
    }
  });

  it('REGRESSION: Validate the NFT List with incorrect account address on the optimism network', async () => {
    if (runTest) {
      try {
        await optimismMainNetSdk.getNftList({
          chainId: Number(process.env.OPTIMISM_CHAINID),
      ...
      console.warn(
        'DUE TO INSUFFICIENT WALLET BALANCE, SKIPPING TEST CASE OF THE HISTORY OF THE NFT LIST WITH INCORRECT ACCOUNT ON THE optimism NETWORK',
      );
    }
  });
});
