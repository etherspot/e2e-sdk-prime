  ...
  it('SMOKE: Validate the Token List on the optimism network', async function () {
    ...
    if (runTest) {
      await customRetryAsync(async function () {
        ...
        let name;
        try {
          tokenLists = await optimismMainNetSdk.getTokenLists();
          ...
          tokenListTokens = await optimismMainNetSdk.getTokenListTokens();
      ...
      console.warn(
        'DUE TO INSUFFICIENT WALLET BALANCE, SKIPPING TEST CASE OF THE TOKEN LIST ON THE optimism NETWORK',
      );
    }
  });
    ...
    if (runTest) {
      await customRetryAsync(async function () {
        ...
        try {
          ...
          rates = await optimismMainNetSdk.fetchExchangeRates(requestPayload);
      ...
      console.warn(
        'DUE TO INSUFFICIENT WALLET BALANCE, SKIPPING TEST CASE OF THE EXCHANGE RATE ON THE optimism NETWORK',
      );
    }
  });

  it('REGRESSION: Validate the NFT List with invalid chainid on the optimism network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        try {
    ...
    if (runTest) {
      await customRetryAsync(async function () {
        try {
          ...
          await optimismMainNetSdk.fetchExchangeRates(requestPayload);
        } catch (e) {
          let error = e.message;
