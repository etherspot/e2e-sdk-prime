...
describe('The PrimeSDK, when transfer a token with optimism network on the MainNet', function () {
  ...
  it('SMOKE: Perform the transfer native token by passing callGasLimit with valid details on the optimism network', async function () {
    ...
    if (runTest) {
      await customRetryAsync(async function () {
        ...
        let op;
        try {
          op = await optimismMainNetSdk.estimate(null, null, 50000);
  ...
  it('REGRESSION: Perform the transfer native token by passing less callGasLimit to the batch while estimate the added transactions to the batch on the optimism network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
        ...
        // passing callGasLimit as 40000 to manually set it
        try {
          await optimismMainNetSdk.estimate(null, null, 40000); // low CallGasLimit
      ...
      console.warn(
        'DUE TO INSUFFICIENT WALLET BALANCE, SKIPPING TEST CASE OF THE SEND NATIVE TOKEN WHEN ADDED LOW CALLGASLIMIT TO THE BATCH ON THE optimism NETWORK',
      );
    }
  });
  ...
  it('REGRESSION: Perform the transfer native token by passing callGasLimit with the incorrect To Address while estimate the added transactions to the batch on the optimism network', async function () {
    ...
    if (runTest) {
      await customRetryAsync(async function () {
        ...
        // passing callGasLimit as 40000 to manually set it
        try {
          await optimismMainNetSdk.estimate(null, null, 40000);
