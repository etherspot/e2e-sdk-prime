...
describe('The PrimeSDK, when transfer a token with matic network on the MainNet', function () {
  ...
  it('SMOKE: Perform the transfer native token by passing callGasLimit with valid details on the matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
  ...
  it('SMOKE: Perform the concurrent userops with valid details on the matic network', async function () {
    ...
    if (runTest) {
      ...
      await customRetryAsync(async function () {
        ...
        try {
          while (--concurrentUseropsCount >= 0) {
            const op = await maticMainNetSdk.estimate({ key: concurrentUseropsCount });
            userops.push(op);
          }

          console.log("Sending userops...");
          for (const op of userops) {
            const uoHash = await maticMainNetSdk.send(op);
        ...
        try {
          ...
          while ((userOpsReceipts.some(receipt => receipt == null)) && (Date.now() < timeout)) {
            ...
            for (let i = 0; i < uoHashes.length; ++i) {
              ...
              userOpsReceipts[i] = await maticMainNetSdk.getUserOpReceipt(uoHash);
            }
          }
      ...
      console.warn(
        'DUE TO INSUFFICIENT WALLET BALANCE, SKIPPING TEST CASE OF THE CONCURRENT USEROPS ON THE matic NETWORK',
      );
    }
  });
  ...
  it('REGRESSION: Perform the transfer native token by passing callGasLimit with the incorrect To Address while estimate the added transactions to the batch on the matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
  ...
  it('REGRESSION: Perform the transfer native token by passing callGasLimit with the invalid To Address i.e. missing character while estimate the added transactions to the batch on the matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
  ...
  it('REGRESSION: Perform the transfer native token by passing callGasLimit without adding transaction to the batch while estimate the added transactions to the batch on the matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
  ...
  it('REGRESSION: Perform the concurrent userops with invalid concurrentUseropsCount on the matic network', async function () {
    // NOTE: assume the sender wallet is deployed
  ...
  it('REGRESSION: Perform the concurrent userops without concurrentUseropsCount on the matic network', async function () {
    // NOTE: assume the sender wallet is deployed
  ...
  it('REGRESSION: Perform the concurrent userops with non deployed address on the matic network', async function () {
    var test = this;
    if (runTest) {
