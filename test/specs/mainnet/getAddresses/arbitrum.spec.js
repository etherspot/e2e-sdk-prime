      ...
      try {
        arbitrumMainNetSdk = new PrimeSdk(
          { privateKey: process.env.PRIVATE_KEY },
          {
            ...
            factoryWallet: Factory.ZERO_DEV, bundlerProvider: new EtherspotBundler(Number(data.arbitrum_chainid), process.env.PORTAL_API_KEY)
          },
        );
      ...
      try {
        arbitrumMainNetSdk = new PrimeSdk(
          { privateKey: process.env.PRIVATE_KEY },
          {
            chainId: Number(data.arbitrum_chainid),
            projectKey: process.env.PROJECT_KEY,
            factoryWallet: Factory.SIMPLE_ACCOUNT, bundlerProvider: new EtherspotBundler(Number(data.arbitrum_chainid), process.env.PORTAL_API_KEY)
          },
        );
    ...
    await customRetryAsync(async function () {
      ...
      try {
        arbitrumMainNetSdk1 = new PrimeSdk(
          ...
          {
            chainId: Number(data.arbitrum_chainid),
            projectKey: process.env.PROJECT_KEY, index: 1, bundlerProvider: new EtherspotBundler(Number(data.arbitrum_chainid), process.env.PORTAL_API_KEY)
          });
