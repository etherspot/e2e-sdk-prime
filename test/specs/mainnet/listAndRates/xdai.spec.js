...
describe('The PrimeSDK, when get the NFT List, Token List and Exchange Rates details with xdai network on the MainNet', function () {
  ...
  it('SMOKE: Validate the Token List on the xdai network', async function () {
    ...
    if (runTest) {
      ...
      console.warn(
        'DUE TO INSUFFICIENT WALLET BALANCE, SKIPPING TEST CASE OF THE TOKEN LIST ON THE xdai NETWORK',
      );
    }
  });
  ...
  it('REGRESSION: Validate the Token List with different argument in the on the xdai network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        let tokenLists;
