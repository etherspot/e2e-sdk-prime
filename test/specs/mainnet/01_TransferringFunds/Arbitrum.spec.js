...
describe("The SDK, when transfer a token with arbitrum network on the MainNet", () => {
  ...
  xit("Regression: Perform the transfer native token with the same To Address i.e. sender address while sign the added transactions to the batch on the arbitrum network", async () => {
    // clear the transaction batch
    try {
      await arbitrumMainNetSdk.clearUserOpsFromBatch();
    } catch (e) {
