...
let xdaiMainNetSdk_old;
let xdaiMainNetSdk;
let primeAccountAddress;

describe('Perform the precondition for new wallet generation', function () {
  it('PRECONDITION1: Create random private key on the xdai network', async function () {
    ...
    testUtils.setPrivateKey(randomPrivateKeyString);
  });

  it('PRECONDITION2: Initialize the prime sdk for new private key on the xdai network', async function () {
    ...
    await customRetryAsync(async function () {
      ...
      try {
        primeAccountAddress = await xdaiMainNetSdk.getCounterFactualAddress();
  ...
  it('PRECONDITION3: Perform the transfer native token from old wallet to new wallet on the xdai network', async function () {
    var test = this;
    await customRetryAsync(async function () {
      // wait for the execution
      helper.wait(data.mediumTimeout);
      ...
      // initializating sdk
      try {
        xdaiMainNetSdk_old = new PrimeSdk(
      ...
      try {
        await xdaiMainNetSdk_old.clearUserOpsFromBatch();
      } catch (e) {
        console.error(e);
        const eString = e.toString();
        addContext(test, eString);
      ...
      try {
        transactionBatch = await xdaiMainNetSdk_old.addUserOpsToBatch({
      ...
      let op;
      try {
        op = await xdaiMainNetSdk_old.estimate();
      } catch (e) {
        console.error(e);
        const eString = e.toString();
        addContext(test, eString);
      ...
      let uoHash;
      try {
        uoHash = await xdaiMainNetSdk_old.send(op);
      } catch (e) {
        console.error(e);
        const eString = e.toString();
        if (eString === 'Error') {
  ...
  it('PRECONDITION4: Perform the transfer ERC20 token from old wallet to new wallet on the xdai network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        helper.wait(data.mediumTimeout);
  ...
  it('PRECONDITION5: Perform the transfer native token from new wallet to old wallet on the xdai network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // wait for the execution
