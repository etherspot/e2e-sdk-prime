...
describe('The PrimeSDK, when get cross chain quotes and get advance routes LiFi transaction details with arbitrum network on the MainNet.', function () {
  ...
  xit('SMOKE: Perform the transfer native token with paymaster on the arbitrum network', async function () {
    var test = this;
    if (runTest) {
        ...
        if (utils.isAddress(paymasterAddress)) {
          ...
          try {
            ...
            while (userOpsReceipt1 == null && Date.now() < timeout1) {
              ...
              userOpsReceipt1 =
                await arbitrumMainNetSdk.getUserOpReceipt(uoHash1);
            }
          ...
          try {
            ...
            while (userOpsReceipt == null && Date.now() < timeout) {
              await Helper.wait(1000);
              userOpsReceipt =
                await arbitrumMainNetSdk.getUserOpReceipt(uoHash);
            }
  ...
  xit('REGRESSION: Perform the transfer native token with invalid paymaster url on the arbitrum network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
  ...
  xit('REGRESSION: Perform the transfer native token without paymaster url on the arbitrum network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
  ...
  xit('REGRESSION: Perform the transfer native token with invalid API Key on the arbitrum network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
  ...
  xit('REGRESSION: Perform the transfer native token with incorrect API Key on the arbitrum network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
  ...
  xit('REGRESSION: Perform the transfer native token without API Key on the arbitrum network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
  ...
  xit('REGRESSION: Perform the transfer native token with invalid context on the arbitrum network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
  ...
  xit('REGRESSION: Perform the transfer native token without context on the arbitrum network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
  ...
  xit('REGRESSION: Perform the transfer token on arka pimlico paymaster without API Key in queryString on the arbitrum network', async function () {
    var test = this;
    let arka_url = data.paymaster_arka;
    let queryString = `?chainId=${Number(process.env.ARBITRUM_CHAINID)}`; // without API Key in queryString
    if (runTest) {
      await customRetryAsync(async function () {
        let returnedValue;
  ...
  xit('REGRESSION: Perform the transfer token on arka pimlico paymaster with invalid ChainID in queryString on the arbitrum network', async function () {
    ...
    let arka_url = data.paymaster_arka;
    let queryString = `?apiKey=${process.env.API_KEY}&chainId=${Number(
      process.env.INVALID_ARBITRUM_CHAINID,
  ...
  xit('REGRESSION: Perform the transfer token on arka pimlico paymaster with invalid Api Key while estimate the transactions on the arbitrum network', async function () {
    ...
    }&chainId=${Number(process.env.ARBITRUM_CHAINID)}`; // invalid API Key in queryString
    if (runTest) {
      await customRetryAsync(async function () {
        let returnedValue;
        let paymasterAddress;
