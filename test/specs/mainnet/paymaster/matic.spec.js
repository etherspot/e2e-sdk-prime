...
describe('The PrimeSDK, when transaction with arka and pimlico paymasters with matic network on the MainNet.', function () {
  ...
  it('REGRESSION: Perform the transfer native token with invalid arka paymaster url on the matic network', async function () {
    ...
    if (runTest) {
      ...
      console.warn(
        'DUE TO INSUFFICIENT WALLET BALANCE, SKIPPING TEST CASE OF THE ARKA FUNCTION WITH PAYMASTER ON THE matic NETWORK',
      );
    }
  });

  it('REGRESSION: Perform the transfer native token without arka paymaster url on the matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // clear the transaction batch
