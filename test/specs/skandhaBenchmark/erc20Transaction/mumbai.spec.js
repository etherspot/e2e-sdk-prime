...
describe('Determine benchmarks of skandha with erc20 token transaction on the mumbai network', function () {

    it('Perform the transfer ERC20 token on the mumbai network for determine benchmarks of skandha', async function () {
        var test = this;
        await customRetryAsync(async function () {
            ...
            try {
                mumbaiTestNetSdk = new PrimeSdk(
                    ...
                    { chainId: Number(data.mumbai_chainid), projectKey: process.env.PROJECT_KEY_TESTNET },
                );
            } catch (e) {
                console.error(e);
            ...
            for (let j = 0; j < txCount; j++) {
                ...
                try {
                 ...
                 try {
                    userOpsBatch.push(await mumbaiTestNetSdk.addUserOpsToBatch({ to: data.tokenAddress_mumbaiUSDC, data: transactionData }));
                } catch (e) {
                    console.error(e);
                    const eString = e.toString();
                    addContext(test, eString);
