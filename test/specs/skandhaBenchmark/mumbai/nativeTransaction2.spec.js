...
describe('The PrimeSDK, Determine benchmarks of Skandha with native token transaction from wallet 2 on the mumbai network', function () {

    it('Perform the concurrent native token transactions from wallet 2 on the mumbai network for determine benchmarks of skandha', async function () {
        // NOTE: assume the sender wallet is deployed
        var test = this;
        let mumbaiTestNetSdk2;
        let mumbaiConcurrentUseropsCount_native2 = 10;

        await customRetryAsync(async function () {
            // initializating sdk for wallet 2
            try {
                mumbaiTestNetSdk2 = new PrimeSdk(
            ...
            try {
                await mumbaiTestNetSdk2.clearUserOpsFromBatch();
            } catch (e) {
                console.error(e);
                const eString = e.toString();
                addContext(test, eString);
            ...
            try {
                await mumbaiTestNetSdk2.addUserOpsToBatch({
                    to: data.recipient1,
            ...
            try {
                while (--mumbaiConcurrentUseropsCount_native2 >= 0) {
                    op_native2 = await mumbaiTestNetSdk2.estimate({ key: mumbaiConcurrentUseropsCount_native2 });
            ...
            try {
                for (const op_native2 of userops_native2) {
                    const uoHash_native2 = await mumbaiTestNetSdk2.send(op_native2);
                    uoHashes_native2.push(uoHash_native2);
                    submitCount_native2++;
                }

                addContext(test, 'Total ' + submitCount_native2 + ' native token transactions are submitted on mumbai network from wallet 2 out of ' + estimationCount_native2 + ' estimated native token transactions')
                console.log('Total ' + submitCount_native2 + ' native token transactions are submitted on mumbai network from wallet 2 out of ' + estimationCount_native2 + ' estimated native token transactions')
            } catch (e) {
                addContext(test, 'Total ' + submitCount_native2 + ' native token transactions are submitted on mumbai network from wallet 2 out of ' + estimationCount_native2 + ' estimated native token transactions')
                console.log('Total ' + submitCount_native2 + ' native token transactions are submitted on mumbai network from wallet 2 out of ' + estimationCount_native2 + ' estimated native token transactions')
            }
            ...
            try {
                ...
                while ((userOpsReceipts_native2.some(receipt => receipt == null)) && (Date.now() < timeout)) {
                    for (let i = 0; i < uoHashes_native2.length; ++i) {
                        ...
                        userOpsReceipts_native2[i] = await mumbaiTestNetSdk2.getUserOpReceipt(uoHash_native2);
                        if (userOpsReceipts_native2[i] != null) {
                            ...
                            addContext(test, decimalNumber_native2 + ' is the block number of the native token transaction count ' + (i + 1) + ' on the mumbai network of wallet 2.')
                            console.log(decimalNumber_native2 + ' is the block number of the native token transaction count ' + (i + 1) + ' on the mumbai network of wallet 2.');
                        }
                    }
                }
            } catch (e) {
