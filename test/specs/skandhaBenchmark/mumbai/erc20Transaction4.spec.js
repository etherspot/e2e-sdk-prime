...
describe('The PrimeSDK, Determine benchmarks on Skandha with transaction from wallet 4 on the mumbai network', function () {

    it('Perform the concurrent erc20 token transactions from wallet 4 on the mumbai network for determine benchmarks of skandha', async function () {
        // NOTE: assume the sender wallet is deployed
        var test = this;
        let mumbaiTestNetSdk4;
        let mumbaiConcurrentUseropsCount_erc204 = 10;
        ...
        await customRetryAsync(async function () {
            ...
            try {
                while (--mumbaiConcurrentUseropsCount_erc204 >= 0) {
                    op_erc204 = await mumbaiTestNetSdk4.estimate({ key: mumbaiConcurrentUseropsCount_erc204 });
            ...
            try {
                for (const op_erc204 of userops_erc204) {
                    const uoHash_erc204 = await mumbaiTestNetSdk4.send(op_erc204);
                    uoHashes_erc204.push(uoHash_erc204);
                    submitCount_erc204++;
                }

                addContext(test, 'Total ' + submitCount_erc204 + ' erc20 token transactions are submitted on mumbai network from wallet 4 out of ' + estimationCount_erc204 + ' estimated erc20 token transactions')
                console.log('Total ' + submitCount_erc204 + ' erc20 token transactions are submitted on mumbai network from wallet 4 out of ' + estimationCount_erc204 + ' estimated erc20 token transactions')
            } catch (e) {
                addContext(test, 'Total ' + submitCount_erc204 + ' erc20 token transactions are submitted on mumbai network from wallet 4 out of ' + estimationCount_erc204 + ' estimated erc20 token transactions')
                console.log('Total ' + submitCount_erc204 + ' erc20 token transactions are submitted on mumbai network from wallet 4 out of ' + estimationCount_erc204 + ' estimated erc20 token transactions')
            }
            ...
            try {
                ...
                while ((userOpsReceipts_erc204.some(receipt => receipt == null)) && (Date.now() < timeout)) {
                    for (let i = 0; i < uoHashes_erc204.length; ++i) {
                        ...
                        userOpsReceipts_erc204[i] = await mumbaiTestNetSdk4.getUserOpReceipt(uoHash_erc204);
                        if (userOpsReceipts_erc204[i] != null) {
                            ...
                            addContext(test, decimalNumber_erc204 + ' is the block number of the erc20 token transaction count ' + (i + 1) + ' on the mumbai network of wallet 4.')
                            console.log(decimalNumber_erc204 + ' is the block number of the erc20 token transaction count ' + (i + 1) + ' on the mumbai network of wallet 4.');
                        }
                    }
                }
            } catch (e) {
