...
describe('The PrimeSDK, Determine benchmarks of Skandha with erc20 token transaction from wallet 1 on the mumbai network', function () {

    it('Perform the concurrent erc20 token transactions from wallet 1 on the mumbai network for determine benchmarks of skandha', async function () {
        // NOTE: assume the sender wallet is deployed
        var test = this;
        let mumbaiTestNetSdk1;
        let mumbaiConcurrentUseropsCount_erc201 = 10;
        ...
        await customRetryAsync(async function () {
            ...
            try {
                while (--mumbaiConcurrentUseropsCount_erc201 >= 0) {
                    op_erc201 = await mumbaiTestNetSdk1.estimate({ key: mumbaiConcurrentUseropsCount_erc201 });
            ...
            try {
                for (const op_erc201 of userops_erc201) {
                    const uoHash_erc201 = await mumbaiTestNetSdk1.send(op_erc201);
                    uoHashes_erc201.push(uoHash_erc201);
                    submitCount_erc201++;
                }

                addContext(test, 'Total ' + submitCount_erc201 + ' erc20 token transactions are submitted on mumbai network from wallet 1 out of ' + estimationCount_erc201 + ' estimated erc20 token transactions')
                console.log('Total ' + submitCount_erc201 + ' erc20 token transactions are submitted on mumbai network from wallet 1 out of ' + estimationCount_erc201 + ' estimated erc20 token transactions')
            } catch (e) {
                addContext(test, 'Total ' + submitCount_erc201 + ' erc20 token transactions are submitted on mumbai network from wallet 1 out of ' + estimationCount_erc201 + ' estimated erc20 token transactions')
                console.log('Total ' + submitCount_erc201 + ' erc20 token transactions are submitted on mumbai network from wallet 1 out of ' + estimationCount_erc201 + ' estimated erc20 token transactions')
            }
            ...
            try {
                ...
                while ((userOpsReceipts_erc201.some(receipt => receipt == null)) && (Date.now() < timeout)) {
                    for (let i = 0; i < uoHashes_erc201.length; ++i) {
                        ...
                        userOpsReceipts_erc201[i] = await mumbaiTestNetSdk1.getUserOpReceipt(uoHash_erc201);
                        if (userOpsReceipts_erc201[i] != null) {
                            ...
                            addContext(test, decimalNumber_erc201 + ' is the block number of the erc20 token transaction count ' + (i + 1) + ' on the mumbai network of wallet 1.')
                            console.log(decimalNumber_erc201 + ' is the block number of the erc20 token transaction count ' + (i + 1) + ' on the mumbai network of wallet 1.');
                        }
                    }
                }
            } catch (e) {
