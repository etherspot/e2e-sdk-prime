...
describe('The PrimeSDK, Determine benchmarks of Skandha with erc20 token transaction from wallet 2 on the mumbai network', function () {

    it('Perform the concurrent erc20 token transactions from wallet 2 on the mumbai network for determine benchmarks of skandha', async function () {
        // NOTE: assume the sender wallet is deployed
        var test = this;
        let mumbaiTestNetSdk2;
        let mumbaiConcurrentUseropsCount_erc202 = 10;
        ...
        await customRetryAsync(async function () {
            ...
            try {
                while (--mumbaiConcurrentUseropsCount_erc202 >= 0) {
                    op_erc202 = await mumbaiTestNetSdk2.estimate({ key: mumbaiConcurrentUseropsCount_erc202 });
            ...
            try {
                for (const op_erc202 of userops_erc202) {
                    const uoHash_erc202 = await mumbaiTestNetSdk2.send(op_erc202);
                    uoHashes_erc202.push(uoHash_erc202);
                    submitCount_erc202++;
                }

                addContext(test, 'Total ' + submitCount_erc202 + ' erc20 token transactions are submitted on mumbai network from wallet 2 out of ' + estimationCount_erc202 + ' estimated erc20 token transactions')
                console.log('Total ' + submitCount_erc202 + ' erc20 token transactions are submitted on mumbai network from wallet 2 out of ' + estimationCount_erc202 + ' estimated erc20 token transactions')
            } catch (e) {
                addContext(test, 'Total ' + submitCount_erc202 + ' erc20 token transactions are submitted on mumbai network from wallet 2 out of ' + estimationCount_erc202 + ' estimated erc20 token transactions')
                console.log('Total ' + submitCount_erc202 + ' erc20 token transactions are submitted on mumbai network from wallet 2 out of ' + estimationCount_erc202 + ' estimated erc20 token transactions')
            }
            ...
            try {
                ...
                while ((userOpsReceipts_erc202.some(receipt => receipt == null)) && (Date.now() < timeout)) {
                    for (let i = 0; i < uoHashes_erc202.length; ++i) {
                        ...
                        userOpsReceipts_erc202[i] = await mumbaiTestNetSdk2.getUserOpReceipt(uoHash_erc202);
                        if (userOpsReceipts_erc202[i] != null) {
                            ...
                            addContext(test, decimalNumber_erc202 + ' is the block number of the erc20 token transaction count ' + (i + 1) + ' on the mumbai network of wallet 2.')
                            console.log(decimalNumber_erc202 + ' is the block number of the erc20 token transaction count ' + (i + 1) + ' on the mumbai network of wallet 2.');
                        }
                    }
                }
            } catch (e) {
