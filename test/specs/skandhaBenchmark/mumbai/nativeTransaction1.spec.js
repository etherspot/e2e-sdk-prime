...
describe('The PrimeSDK, Determine benchmarks of Skandha with native token transaction from wallet 1 on the mumbai network', function () {

    it('Perform the concurrent native token transactions from wallet 1 on the mumbai network for determine benchmarks of skandha', async function () {
        // NOTE: assume the sender wallet is deployed
        var test = this;
        let mumbaiTestNetSdk1;
        let mumbaiConcurrentUseropsCount_native1 = 10;
        ...
        await customRetryAsync(async function () {
            ...
            try {
                await mumbaiTestNetSdk1.clearUserOpsFromBatch();
            } catch (e) {
                console.error(e);
                const eString = e.toString();
                addContext(test, eString);
            ...
            try {
                await mumbaiTestNetSdk1.addUserOpsToBatch({
                    to: data.recipient1,
            ...
            try {
                while (--mumbaiConcurrentUseropsCount_native1 >= 0) {
                    op_native1 = await mumbaiTestNetSdk1.estimate({ key: mumbaiConcurrentUseropsCount_native1 });
            ...
            try {
                for (const op_native1 of userops_native1) {
                    const uoHash_native1 = await mumbaiTestNetSdk1.send(op_native1);
                    uoHashes_native1.push(uoHash_native1);
                    submitCount_native1++;
                }

                addContext(test, 'Total ' + submitCount_native1 + ' native token transactions are submitted on mumbai network from wallet 1 out of ' + estimationCount_native1 + ' estimated native token transactions')
                console.log('Total ' + submitCount_native1 + ' native token transactions are submitted on mumbai network from wallet 1 out of ' + estimationCount_native1 + ' estimated native token transactions')
            } catch (e) {
                addContext(test, 'Total ' + submitCount_native1 + ' native token transactions are submitted on mumbai network from wallet 1 out of ' + estimationCount_native1 + ' estimated native token transactions')
                console.log('Total ' + submitCount_native1 + ' native token transactions are submitted on mumbai network from wallet 1 out of ' + estimationCount_native1 + ' estimated native token transactions')
            }
            ...
            try {
                ...
                while ((userOpsReceipts_native1.some(receipt => receipt == null)) && (Date.now() < timeout)) {
                    for (let i = 0; i < uoHashes_native1.length; ++i) {
                        ...
                        userOpsReceipts_native1[i] = await mumbaiTestNetSdk1.getUserOpReceipt(uoHash_native1);
                        if (userOpsReceipts_native1[i] != null) {
                            ...
                            addContext(test, decimalNumber_native1 + ' is the block number of the native token transaction count ' + (i + 1) + ' on the mumbai network of wallet 1.')
                            console.log(decimalNumber_native1 + ' is the block number of the native token transaction count ' + (i + 1) + ' on the mumbai network of wallet 1.');
                        }
                    }
                }
            } catch (e) {
