...
describe('TThe PrimeSDK, Determine benchmarks of Skandha with erc20 token transaction from wallet 3 on the mumbai network', function () {

    it('Perform the concurrent erc20 token transactions from wallet 3 on the mumbai network for determine benchmarks of skandha', async function () {
        // NOTE: assume the sender wallet is deployed
        var test = this;
        let mumbaiTestNetSdk3;
        let mumbaiConcurrentUseropsCount_erc203 = 10;
        ...
        await customRetryAsync(async function () {
            ...
            try {
                while (--mumbaiConcurrentUseropsCount_erc203 >= 0) {
                    op_erc203 = await mumbaiTestNetSdk3.estimate({ key: mumbaiConcurrentUseropsCount_erc203 });
            ...
            try {
                for (const op_erc203 of userops_erc203) {
                    const uoHash_erc203 = await mumbaiTestNetSdk3.send(op_erc203);
                    uoHashes_erc203.push(uoHash_erc203);
                    submitCount_erc203++;
                }

                addContext(test, 'Total ' + submitCount_erc203 + ' erc20 token transactions are submitted on mumbai network from wallet 3 out of ' + estimationCount_erc203 + ' estimated erc20 token transactions')
                console.log('Total ' + submitCount_erc203 + ' erc20 token transactions are submitted on mumbai network from wallet 3 out of ' + estimationCount_erc203 + ' estimated erc20 token transactions')
            } catch (e) {
                addContext(test, 'Total ' + submitCount_erc203 + ' erc20 token transactions are submitted on mumbai network from wallet 3 out of ' + estimationCount_erc203 + ' estimated erc20 token transactions')
                console.log('Total ' + submitCount_erc203 + ' erc20 token transactions are submitted on mumbai network from wallet 3 out of ' + estimationCount_erc203 + ' estimated erc20 token transactions')
            }
            ...
            try {
                ...
                while ((userOpsReceipts_erc203.some(receipt => receipt == null)) && (Date.now() < timeout)) {
                    for (let i = 0; i < uoHashes_erc203.length; ++i) {
                        ...
                        userOpsReceipts_erc203[i] = await mumbaiTestNetSdk3.getUserOpReceipt(uoHash_erc203);
                        if (userOpsReceipts_erc203[i] != null) {
                            ...
                            addContext(test, decimalNumber_erc203 + ' is the block number of the erc20 token transaction count ' + (i + 1) + ' on the mumbai network of wallet 3.')
                            console.log(decimalNumber_erc203 + ' is the block number of the erc20 token transaction count ' + (i + 1) + ' on the mumbai network of wallet 3.');
                        }
                    }
                }
            } catch (e) {
