...
describe('The PrimeSDK, Determine benchmarks of Skandha with native token transaction from wallet 4 on the sepolia network', function () {

    it('Perform the concurrent native token transactions from wallet 4 on the sepolia network for determine benchmarks of skandha', async function () {
        // NOTE: assume the sender wallet is deployed
        var test = this;
        let sepoliaTestNetSdk4;
        let sepoliaConcurrentUseropsCount_native4 = 10;
        ...
        await customRetryAsync(async function () {
            ...
            try {
                while (--sepoliaConcurrentUseropsCount_native4 >= 0) {
                    op_native4 = await sepoliaTestNetSdk4.estimate({ key: sepoliaConcurrentUseropsCount_native4 });
            ...
            try {
                for (const op_native4 of userops_native4) {
                    const uoHash_native4 = await sepoliaTestNetSdk4.send(op_native4);
                    uoHashes_native4.push(uoHash_native4);
                    submitCount_native4++;
                }

                addContext(test, 'Total ' + submitCount_native4 + ' native token transactions are submitted on sepolia network from wallet 4 out of ' + estimationCount_native4 + ' estimated native token transactions')
                console.log('Total ' + submitCount_native4 + ' native token transactions are submitted on sepolia network from wallet 4 out of ' + estimationCount_native4 + ' estimated native token transactions')
            } catch (e) {
                addContext(test, 'Total ' + submitCount_native4 + ' native token transactions are submitted on sepolia network from wallet 4 out of ' + estimationCount_native4 + ' estimated native token transactions')
                console.log('Total ' + submitCount_native4 + ' native token transactions are submitted on sepolia network from wallet 4 out of ' + estimationCount_native4 + ' estimated native token transactions')
            }
            ...
            try {
                ...
                while ((userOpsReceipts_native4.some(receipt => receipt == null)) && (Date.now() < timeout)) {
                    for (let i = 0; i < uoHashes_native4.length; ++i) {
                        ...
                        userOpsReceipts_native4[i] = await sepoliaTestNetSdk4.getUserOpReceipt(uoHash_native4);
                        if (userOpsReceipts_native4[i] != null) {
                            ...
                            addContext(test, decimalNumber_native4 + ' is the block number of the native token transaction count ' + (i + 1) + ' on the sepolia network of wallet 4.')
                            console.log(decimalNumber_native4 + ' is the block number of the native token transaction count ' + (i + 1) + ' on the sepolia network of wallet 4.');
                        }
                    }
                }
            } catch (e) {
