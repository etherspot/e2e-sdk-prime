...
describe('The PrimeSDK, Determine benchmarks of Skandha with erc20 token transaction from wallet 5 on the sepolia network', function () {

    it('Perform the concurrent erc20 token transactions from wallet 5 on the sepolia network for determine benchmarks of skandha', async function () {
        // NOTE: assume the sender wallet is deployed
        var test = this;
        let sepoliaTestNetSdk5;
        let sepoliaConcurrentUseropsCount_erc205 = 10;
        ...
        await customRetryAsync(async function () {
            ...
            try {
                while (--sepoliaConcurrentUseropsCount_erc205 >= 0) {
                    op_erc205 = await sepoliaTestNetSdk5.estimate({ key: sepoliaConcurrentUseropsCount_erc205 });
            ...
            try {
                for (const op_erc205 of userops_erc205) {
                    const uoHash_erc205 = await sepoliaTestNetSdk5.send(op_erc205);
                    uoHashes_erc205.push(uoHash_erc205);
                    submitCount_erc205++;
                }

                addContext(test, 'Total ' + submitCount_erc205 + ' erc20 token transactions are submitted on sepolia network from wallet 5 out of ' + estimationCount_erc205 + ' estimated erc20 token transactions')
                console.log('Total ' + submitCount_erc205 + ' erc20 token transactions are submitted on sepolia network from wallet 5 out of ' + estimationCount_erc205 + ' estimated erc20 token transactions')
            } catch (e) {
                addContext(test, 'Total ' + submitCount_erc205 + ' erc20 token transactions are submitted on sepolia network from wallet 5 out of ' + estimationCount_erc205 + ' estimated erc20 token transactions')
                console.log('Total ' + submitCount_erc205 + ' erc20 token transactions are submitted on sepolia network from wallet 5 out of ' + estimationCount_erc205 + ' estimated erc20 token transactions')
            }
            ...
            try {
                ...
                while ((userOpsReceipts_erc205.some(receipt => receipt == null)) && (Date.now() < timeout)) {
                    for (let i = 0; i < uoHashes_erc205.length; ++i) {
                        ...
                        userOpsReceipts_erc205[i] = await sepoliaTestNetSdk5.getUserOpReceipt(uoHash_erc205);
                        if (userOpsReceipts_erc205[i] != null) {
                            ...
                            addContext(test, decimalNumber_erc205 + ' is the block number of the erc20 token transaction count ' + (i + 1) + ' on the sepolia network of wallet 5.')
                            console.log(decimalNumber_erc205 + ' is the block number of the erc20 token transaction count ' + (i + 1) + ' on the sepolia network of wallet 5.');
                        }
                    }
                }
            } catch (e) {
