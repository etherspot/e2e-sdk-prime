...
describe('The PrimeSDK, Determine benchmarks of Skandha with native token transaction from wallet 3 on the sepolia network', function () {

    it('Perform the concurrent native token transactions from wallet 3 on the sepolia network for determine benchmarks of skandha', async function () {
        // NOTE: assume the sender wallet is deployed
        var test = this;
        let sepoliaTestNetSdk3;
        let sepoliaConcurrentUseropsCount_native3 = 10;
        ...
        await customRetryAsync(async function () {
            ...
            try {
                while (--sepoliaConcurrentUseropsCount_native3 >= 0) {
                    op_native3 = await sepoliaTestNetSdk3.estimate({ key: sepoliaConcurrentUseropsCount_native3 });
            ...
            try {
                for (const op_native3 of userops_native3) {
                    const uoHash_native3 = await sepoliaTestNetSdk3.send(op_native3);
                    uoHashes_native3.push(uoHash_native3);
                    submitCount_native3++;
                }

                addContext(test, 'Total ' + submitCount_native3 + ' native token transactions are submitted on sepolia network from wallet 3 out of ' + estimationCount_native3 + ' estimated native token transactions')
                console.log('Total ' + submitCount_native3 + ' native token transactions are submitted on sepolia network from wallet 3 out of ' + estimationCount_native3 + ' estimated native token transactions')
            } catch (e) {
                addContext(test, 'Total ' + submitCount_native3 + ' native token transactions are submitted on sepolia network from wallet 3 out of ' + estimationCount_native3 + ' estimated native token transactions')
                console.log('Total ' + submitCount_native3 + ' native token transactions are submitted on sepolia network from wallet 3 out of ' + estimationCount_native3 + ' estimated native token transactions')
            }
            ...
            try {
                ...
                while ((userOpsReceipts_native3.some(receipt => receipt == null)) && (Date.now() < timeout)) {
                    for (let i = 0; i < uoHashes_native3.length; ++i) {
                        ...
                        userOpsReceipts_native3[i] = await sepoliaTestNetSdk3.getUserOpReceipt(uoHash_native3);
                        if (userOpsReceipts_native3[i] != null) {
                            ...
                            addContext(test, decimalNumber_native3 + ' is the block number of the native token transaction count ' + (i + 1) + ' on the sepolia network of wallet 3.')
                            console.log(decimalNumber_native3 + ' is the block number of the native token transaction count ' + (i + 1) + ' on the sepolia network of wallet 3.');
                        }
                    }
                }
            } catch (e) {
