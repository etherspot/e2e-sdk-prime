...
describe('Performance testing of Arka Endpoints with Xdai Network', () => {
  it.only('SMOKE: Validate the Deposit endpoint of Arka on Xdai Network', async () => {
    ...
    try {
      const response = await fetch(data.arka_deposit, {
        ...
        body: JSON.stringify({
          params: [data.amount, data.xdai_chainid_testnet, process.env.API_KEY],
        }),
      });
      if (!response.ok) {
  ...
  it('SMOKE: Validate the Whitelist endpoint of Arka on Xdai Network', async () => {
    ...
    try {
      const response = await fetch(data.arka_whitelist, {
        ...
        body: JSON.stringify({
          params: [addresses, data.xdai_chainid_testnet, process.env.API_KEY],
        }),
      });
      if (!response.ok) {
        console.error('Response status:', response.status);
  ...
  it('SMOKE: Validate the Check an Address is Whitelist endpoint of Arka on Xdai Network', async () => {
  ...
  it('SMOKE: Validate the Pimlico Paymaster endpoint of Arka on Xdai Network', async () => {
  ...
  it('SMOKE: Perform the Sponsor a Transaction with Arka and PrimeSDK on the Xdai network', async () => {
    const startTime = performance.now();
